...
async function cacBuocGia() {
  ...
  return docJSON(banGhi?.gia_tri_cau_hinh, []).map((b,i)=>({
    ...b,id:String(b.id??i+1),dang_hoat_dong:b.dang_hoat_dong??1,
  })).filter(b=>b.dang_hoat_dong);
}
const luuBoBuocGia = (duLieu,nguoiCapNhatId) => coSoDuLieu.truyVan(
