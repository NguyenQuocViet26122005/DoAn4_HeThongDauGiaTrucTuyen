...
async function taoDonNguoiThang(phienDauGia, nguoiMuaId, gia, nguon = 'THANG_DAU_GIA', diaChi = undefined) {
...
async function thanhToan(nguoiDung: NguoiDungDangNhap, id, duLieuNhap: unknown = {}) {
  ...
  const ketQua = kiemTra.giaTriLuaChon(dauVao.ket_qua_mo_phong ?? 'THANH_CONG', ['THANH_CONG','THAT_BAI'], 'Kết quả mô phỏng');
  const khoaYeuCau = dauVao.khoa_yeu_cau == null ? null : kiemTra.chuoi(dauVao.khoa_yeu_cau, 'Khóa yêu cầu', 100);
  baoDam(khoaYeuCau == null || /^[a-zA-Z0-9_-]{8,100}$/.test(khoaYeuCau), 400, 'Khóa yêu cầu cần 8–100 ký tự chữ, số, gạch ngang hoặc gạch dưới');
  return coSoDuLieu.giaoDich(async () => {
    const banGhi = batBuocTonTai(await khoDuLieu.khoaDuLieu(kiemTra.id(id)));
    nguoiMua(nguoiDung, banGhi);
    if (
    ...
    if (khoaYeuCau) {
      const daXuLy = await coSoDuLieu.layMot('SELECT don_hang_id FROM thanh_toan WHERE khoa_yeu_cau=?',[khoaYeuCau]);
      if (daXuLy) {
    ...
    if (ketQua === 'THAT_BAI') {
      await khoBanGhi.them('thanh_toan', {don_hang_id:id, phuong_thuc_thanh_toan:'MO_PHONG',so_tien:banGhi.tong_tien,trang_thai:'THAT_BAI',ma_giao_dich:`SIM-${taoMaNgauNhien()}`,khoa_yeu_cau:khoaYeuCau,ngay_het_han:banGhi.han_thanh_toan});
...
async function guiHang(nguoiDung: NguoiDungDangNhap, id, duLieuNhap: unknown) {
  ...
  return coSoDuLieu.giaoDich(async () => {
    ...
    await khoBanGhi.capNhat('don_hang', id, {
      ...
      moc_khieu_nai_chua_nhan: thoiGian.congGiay(thoiGianHienTai, (await cauHinhNghiepVu.docSoCauHinh('BUYER_NON_RECEIPT_DAYS')) * 86400),
...
async function hoanThanhDonDaKhoa(banGhi, nguoiThucHienId) {
  ...
  const cacId = [banGhi.nguoi_mua_id,banGhi.nguoi_ban_id].sort((a,b)=>BigInt(a)<BigInt(b)?-1:1);
  let biHanChe = false;
  for (const id of cacId) {
    const taiKhoan = batBuocTonTai(await cacNguoiDung.layTheoId(id,true));
    if (taiKhoan.trang_thai_tai_khoan !== 'HOAT_DONG') biHanChe = true;
  }
  if (biHanChe) {
    await khoBanGhi.capNhat('don_hang',banGhi.id,{can_admin_xu_ly:1,ly_do_can_xu_ly:'Tài khoản giao dịch bị khóa/tạm ngưng; cần Admin kiểm tra trước khi giải ngân.'});
    return;
  }
...
async function ghiNhanGiaoMuon(banGhi) {
  ...
  await khoBanGhi.capNhat('don_hang',banGhi.id,{can_admin_xu_ly:1,ly_do_can_xu_ly:'Người bán quá hạn gửi hàng; cần Admin kiểm tra.'});
...
async function xuLyDenHan(id) {
  return coSoDuLieu.giaoDich(async () => {
    ...
    if (
      ...
      await taoThongBao(banGhi.nguoi_ban_id, 'DON_KHONG_THANH_TOAN', 'Người thắng không thanh toán', 'Bạn có thể chủ động tạo đề nghị mua tiếp theo từ đơn này.', `/orders/${id}`);
    } else if (
      ['DA_GIAO', 'DANG_KIEM_TRA'].includes(banGhi.trang_thai) &&
