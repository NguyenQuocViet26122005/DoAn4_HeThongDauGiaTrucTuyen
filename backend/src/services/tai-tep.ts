...
baoDam(nguoiDung.trang_thai_nguoi_ban === 'DA_XAC_MINH', 403, 'Cần xác minh người bán');
  ...
  return {
 duong_dan: dich.url, loai: loai.mime, kich_thuoc: tep.size 
};
}
...
async function kiemTraTepSoHuu(giaTri, nhom, nguoiDungId) {
  ...
  try {
    ...
    if (loi.code === 'ENOENT') {
baoDam(false, 400, 'Tệp không tồn tại');
}
    throw loi;
  }
...
async function taiTep(nguoiDung: NguoiDungDangNhap, nhom, chuSoHuu, ten) {
  ...
  try {
    ...
    if (loi.code === 'ENOENT') {
baoDam(false, 404, 'Không tìm thấy tệp');
}
    throw loi;
  }
...
export {
 luu, kiemTraTepSoHuu, taiTep, nhanDangLoaiTep 
};
