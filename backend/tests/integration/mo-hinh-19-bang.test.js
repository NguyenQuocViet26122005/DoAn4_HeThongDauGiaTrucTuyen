...
test('19 bảng: theo dõi trước không được ưu tiên; bỏ theo dõi giữ nguyên cam kết', async () => hoanTac(async () => {
...
test('19 bảng: thanh toán lỗi không giữ tiền; retry một lần và hoàn cả phí vận chuyển', async () => hoanTac(async () => {
  const d = await taoDuLieuKiemThu();
  const id = await phienDauGia(d,{gia_mua_ngay:'24000000',cho_phep_mua_ngay:1,phi_van_chuyen:'35000'});
  ...
  await assert.rejects(tranhChap.mo(d.a,don.id,{ly_do:'CHUA_NHAN_HANG',mo_ta:'Chưa tới mốc 7 ngày'}),{status:409});
  await banGhi.capNhat('don_hang',don.id,{moc_khieu_nai_chua_nhan:thoiGian.congGiay(await db.thoiGianHienTai(),-1)});
  const tc = await tranhChap.mo(d.a,don.id,{ly_do:'CHUA_NHAN_HANG',mo_ta:'Đã quá mốc chưa nhận hàng'});
  await assert.rejects(tranhChap.giaiQuyet(d.admin,tc.id,{ket_qua:'NGUOI_MUA',so_tien_hoan:'24000000',ket_qua_xu_ly:'Thiếu phí vận chuyển'}),{status:400});
  await tranhChap.giaiQuyet(d.admin,tc.id,{ket_qua:'NGUOI_MUA',ket_qua_xu_ly:'Hoàn toàn bộ gồm vận chuyển'});
...
test('19 bảng: tài khoản bị khóa không được tự giải ngân khi hết hạn kiểm tra', async () => hoanTac(async () => {
  const d = await taoDuLieuKiemThu();
  const id = await phienDauGia(d,{gia_mua_ngay:'24000000',cho_phep_mua_ngay:1});
  const {don_hang: don} = await dauGia.muaNgay(d.a,id);
  await donHang.thanhToan(d.a,don.id);
  await donHang.guiHang(d.seller,don.id,{don_vi_van_chuyen:'Kiểm thử',ma_van_don:'KT-KHOA'});
  await donHang.xacNhanDaGiao(d.a,don.id);
  await require('../../dist/services/nguoi-dung').doiTrangThaiTaiKhoan(d.admin,d.seller.id,{trang_thai_tai_khoan:'BI_KHOA',ly_do:'Cần kiểm tra'});
  await banGhi.capNhat('don_hang',don.id,{han_kiem_tra:thoiGian.congGiay(await db.thoiGianHienTai(),-1)});
  ...
  const tc = await tranhChap.mo(d.admin,don.id,{ly_do:'KHAC',mo_ta:'Tiếp nhận đơn cần Admin kiểm tra'});
  await tranhChap.giaiQuyet(d.admin,tc.id,{ket_qua:'NGUOI_MUA',ket_qua_xu_ly:'Hoàn toàn bộ'});
