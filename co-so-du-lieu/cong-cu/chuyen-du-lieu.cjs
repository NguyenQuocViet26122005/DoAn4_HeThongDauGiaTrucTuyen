...
async function cacCot(c, schema, table) {
  return (
    ...
    .filter((r) => !/(STORED|VIRTUAL) GENERATED/.test(r.EXTRA))
    .map((r) => r.COLUMN_NAME);
}
...
async function dauVan(c, schema) {
  ...
  for (const table of await cacBang(c, schema)) {
    ...
    const [rows] = await c.query(
      `SELECT SHA2(CAST(JSON_ARRAY(${cols.map(q).join(',')}) AS CHAR),256) AS ma FROM ${full(schema, table)} ORDER BY id`,
    );

    // Chỉ dấu vân tay, không in/lưu các trường tài khoản hay mức giá bí mật.
    ketQua[table] = {
      so_dong: rows.length,
      ma: crypto.createHash('sha256').update(JSON.stringify(rows)).digest('hex'),
    };
  }
...
async function saoChepDuLieu(c, tu, sang, thuTu) {
  for (const table of thuTu) {
    const cols = await cacCot(c, tu, table);

    await c.query(
      `INSERT INTO ${full(sang, table)} (${cols.map(q).join(',')}) SELECT ${cols.map(q).join(',')} FROM ${full(tu, table)}`,
    );
  }
}
...
async function saoChepViewTrigger(c, tu, sang) {
  ...
  const [views] = await c.execute(
    'SELECT TABLE_NAME FROM information_schema.VIEWS WHERE TABLE_SCHEMA=?',
    [tu],
  );

  for (const v of views) {
    const [[ddl]] = await c.query('SHOW CREATE VIEW ' + full(tu, v.TABLE_NAME));

    await c.query(ddl['Create View'].replaceAll(q(tu) + '.', q(sang) + '.'));
  }

  const [trs] = await c.execute(
    'SELECT TRIGGER_NAME FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA=?',
    [tu],
  );

  for (const t of trs) {
    const [[ddl]] = await c.query('SHOW CREATE TRIGGER ' + full(tu, t.TRIGGER_NAME));

    await c.query(ddl['SQL Original Statement'].replaceAll(q(tu) + '.', q(sang) + '.'));
  }
}
...
async function chuanBi(c) {
  assert(
    !fs.existsSync(tepTrangThai),
    'KIEM_TRA: Đã có trạng thái chuyển đổi; không tạo thêm bản sao tự động.',
  );
  ...
  for (const [ten, r] of Object.entries(seed)) {
    assert.equal(
      r.so_dong,
      ten === 'danh_muc' ? 5 : ten === 'cau_hinh_he_thong' ? 8 : 0,
      'KIEM_TRA: Database thiết kế có dữ liệu ngoài cấu hình mẫu.',
    );
  }

  const backup =
    'doan4_daugia_sao_luu_' + new Date().toISOString().replace(/[-:T]/g, '').slice(0, 14);
  const truoc = await dauVan(c, nguon);
  const thuTu = await thuTuBangCu();

  assert.equal(thuTu.length, 27);
  await c.query(
    'CREATE DATABASE ' + q(backup) + ' CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci',
  );
  await saoChepCauTruc(c, nguon, backup);
  await c.beginTransaction();
  try {
    await saoChepDuLieu(c, nguon, backup, thuTu);
    assert.deepEqual(await dauVan(c, backup), truoc);
    assert.deepEqual(await dauVan(c, nguon), truoc);
    await c.commit();
  } catch (e) {
    await c.rollback();
    throw e;
  ...
  const trangThai = {
    giai_doan: 'DA_SAO_LUU',
    nguon,
    dich,
    backup,
    thu_tu_bang_cu: thuTu,
    dau_van_nguon: truoc,
...
async function chuyenThu(c, t) {
  ...
  try {
    ...
    for (const ten of bangMoi) {
      ...
      await c.query(
        `INSERT INTO ${full(dich, ten)} (${cols.map(q).join(',')}${extra}) SELECT ${cols.map((k) => 'o.' + q(k)).join(',')}${selectExtra} FROM ${full(b, ten)} o ORDER BY o.id`,
      );
    }
    ...
    for (const d of dm) {
      const [a] = await c.execute(
        'SELECT * FROM ' +
          full(b, 'thuoc_tinh_danh_muc') +
          ' WHERE danh_muc_id=? ORDER BY thu_tu,id',
        [d.id],
      );

      for (const x of a)
        if (x.lua_chon_json) {
          x.lua_chon_json = giaTriJSON(x.lua_chon_json);
        }
      await c.execute(
        'UPDATE ' +
          full(dich, 'danh_muc') +
          ' SET cau_hinh_thuoc_tinh=?,ngay_cap_nhat=ngay_cap_nhat WHERE id=?',
        [JSON.stringify(a), d.id],
      );
    }
    ...
    for (const p of sp) {
      const [v] = await c.execute(
        `SELECT g.*,t.ten_thuoc_tinh,t.khoa_thuoc_tinh,t.kieu_nhap,t.don_vi,t.thu_tu FROM ${full(b, 'gia_tri_thuoc_tinh_san_pham')} g JOIN ${full(b, 'thuoc_tinh_danh_muc')} t ON t.id=g.thuoc_tinh_id WHERE g.san_pham_id=? ORDER BY t.thu_tu,t.id`,
        [p.id],
      );
      const obj = Object.fromEntries(v.map((x) => [x.khoa_thuoc_tinh, x]));

      await c.execute(
        'UPDATE ' +
          full(dich, 'san_pham') +
          ' SET thuoc_tinh_json=?,ngay_cap_nhat=ngay_cap_nhat WHERE id=?',
        [JSON.stringify(obj), p.id],
      );
    }
    ...
    await c.execute(
      `INSERT INTO ${full(dich, 'cau_hinh_he_thong')} (khoa_cau_hinh,gia_tri_cau_hinh,kieu_du_lieu,mo_ta) VALUES ('BUOC_GIA',?,'JSON','Bộ bước giá chuyển nguyên từ bảng cũ')`,
      [JSON.stringify(buoc)],
    );
    await c.query(
      `INSERT INTO ${full(dich, 'cau_hinh_he_thong')} (khoa_cau_hinh,gia_tri_cau_hinh,kieu_du_lieu,mo_ta) VALUES ('BUYER_NON_RECEIPT_DAYS','7','SO','Mốc khiếu nại chưa nhận từ ngày khai báo gửi')`,
    );
    ...
    await c.query(
      `UPDATE ${full(dich, 'tham_gia_phien')} t JOIN ${full(b, 'danh_sach_theo_doi')} w ON w.phien_dau_gia_id=t.phien_dau_gia_id AND w.nguoi_dung_id=t.nguoi_dung_id SET t.dang_theo_doi=1,t.ngay_theo_doi=w.ngay_tao,t.ngay_cap_nhat=t.ngay_cap_nhat`,
    );
    ...
    const [[maxAnh]] = await c.query(
      'SELECT COALESCE(MAX(id),0) AS id FROM ' + full(b, 'hinh_anh_san_pham'),
    );

    await c.execute(
      `INSERT INTO ${full(dich, 'tep_dinh_kem')} (id,loai_tep,tranh_chap_id,nguoi_tai_len_id,duong_dan_tep,loai_noi_dung,mo_ta,ngay_tao)
      SELECT id+?,'BANG_CHUNG_TRANH_CHAP',tranh_chap_id,nguoi_tai_len_id,duong_dan_tep,loai_bang_chung,mo_ta,ngay_tao FROM ${full(b, 'bang_chung_tranh_chap')}`,
      [maxAnh.id],
    );
    ...
    await c.query(
      `UPDATE ${full(dich, 'phien_dau_gia')} p JOIN (SELECT phien_dau_gia_id,MIN(phi_van_chuyen) AS phi FROM ${full(b, 'don_hang')} GROUP BY phien_dau_gia_id) d ON d.phien_dau_gia_id=p.id SET p.phi_van_chuyen=d.phi,p.ngay_cap_nhat=p.ngay_cap_nhat`,
    );
    ...
    for (const ten of bangMoi) {
      ...
      const cols = await cacCot(c, b, ten);
      const [khac] = await c.query(
        `SELECT s.id FROM ${full(b, ten)} s LEFT JOIN ${full(dich, ten)} d ON d.id=s.id WHERE d.id IS NULL OR NOT (${cols.map((k) => `s.${q(k)} <=> d.${q(k)}`).join(' AND ')})`,
      );

      assert.equal(khac.length, 0, 'KIEM_TRA: Cột lịch sử thay đổi ở ' + ten);
    }

    const [[camKet]] = await c.query(
      `SELECT COUNT(*) AS sai FROM ${full(b, 'muc_gia_toi_da')} m LEFT JOIN ${full(dich, 'tham_gia_phien')} t ON t.phien_dau_gia_id=m.phien_dau_gia_id AND t.nguoi_dung_id=m.nguoi_tra_gia_id WHERE t.id IS NULL OR t.gia_toi_da<>m.gia_toi_da OR t.thoi_gian_dat_gia_toi_da<>m.thoi_gian_dat_gia_toi_da`,
    );

    assert.equal(Number(camKet.sai), 0);
    await c.commit();
    t.giai_doan = 'DA_CHUYEN_THU';
    t.dich_sau_chuyen = await dauVan(c, dich);
    t.anh_id_bang_chung_cong_them = String(maxAnh.id);
    t.ngay_chuyen_thu = new Date().toISOString();
...
async function xoaCauTrucDeThay(c, schema, expected) {
  ...
  assert(
    ds.every((t) => expected.includes(t)),
    'KIEM_TRA: Có bảng ngoài phạm vi đã sao lưu.',
  );
  ...
  for (const x of v) {
    await c.query('DROP VIEW ' + full(schema, x.TABLE_NAME));
  }
  await c.query('SET FOREIGN_KEY_CHECKS=0');
  try {
    for (const ten of ds) {
      await c.query('DROP TABLE ' + full(schema, ten));
    }
  } finally {
    await c.query('SET FOREIGN_KEY_CHECKS=1');
  }
...
async function apDung(c) {
  ...
  try {
    ...
    await saoChepCauTruc(c, t.backup, nguon);
    await c.beginTransaction();
    await saoChepDuLieu(c, t.backup, nguon, t.thu_tu_bang_cu);
    await c.commit();
    await saoChepViewTrigger(c, t.backup, nguon);
    assert.deepEqual(await dauVan(c, nguon), t.dau_van_nguon);
    throw e;
  }
}
...
(async () => {
  ...
  try {
    ...
    if (process.argv[2] === '--chuan-bi') {
      await chuanBi(c);
    } else if (process.argv[2] === '--thu-lai') {
      const t = JSON.parse(fs.readFileSync(tepTrangThai, 'utf8'));

      assert.equal(t.giai_doan, 'DA_SAO_LUU');
      await chuyenThu(c, t);
    } else if (process.argv[2] === '--ap-dung') {
      await apDung(c);
    } else {
      throw new Error('KIEM_TRA: Dùng --chuan-bi, --thu-lai hoặc --ap-dung.');
    }
  } finally {
    c.release();
  }
  ...
  .catch((e) => {
    console.error(
      e.code || e.name,
      e.message?.startsWith('KIEM_TRA:') ? e.message : 'Chi tiết dữ liệu không được in.',
    );
    process.exitCode = 1;
  })
  .finally(() => pool.end());
