...
const trangThai = JSON.parse(fs.readFileSync(path.join(__dirname, '../trang-thai-chuyen-doi.json'), 'utf8'));
...
async function main() {
  ...
  try {
    ...
    await doiChieu('ảnh sản phẩm', `SELECT id,san_pham_id,duong_dan_anh,la_anh_chinh,thu_tu,ngay_tao FROM ${nguon}.hinh_anh_san_pham ORDER BY id`,
    ...
    assert(/^\d+$/.test(bu));
    await doiChieu('bằng chứng riêng tư', `SELECT CAST(id AS CHAR) AS id,tranh_chap_id,nguoi_tai_len_id,duong_dan_tep,loai_bang_chung,mo_ta,ngay_tao FROM ${nguon}.bang_chung_tranh_chap ORDER BY id`,
    ...
    await doiChieu('theo dõi', `SELECT phien_dau_gia_id,nguoi_dung_id,ngay_tao FROM ${nguon}.danh_sach_theo_doi ORDER BY phien_dau_gia_id,nguoi_dung_id`,
    ...
    await doiChieu('cam kết và ưu tiên (không xuất giá trị)', `SELECT id,phien_dau_gia_id,nguoi_tra_gia_id,gia_toi_da,thoi_gian_dat_gia_toi_da,ngay_tao,ngay_cap_nhat FROM ${nguon}.muc_gia_toi_da ORDER BY id`,
    ...
    await doiChieu('vận chuyển', `SELECT * FROM ${nguon}.van_chuyen ORDER BY id`,
    ...
    await doiChieu('giữ tiền', `SELECT * FROM ${nguon}.giu_tien_trung_gian ORDER BY id`,
    ...
    await doiChieu('yêu cầu hủy', `SELECT * FROM ${nguon}.yeu_cau_huy_phien ORDER BY id`,
    ...
    for (const [bangCu,bangMoi,cot] of [['thuoc_tinh_danh_muc','danh_muc','cau_hinh_thuoc_tinh'],['gia_tri_thuoc_tinh_san_pham','san_pham','thuoc_tinh_json']]) {
      ...
      for (const ban of cu) {
        ...
        assert(m);
        for (const khoa of Object.keys(ban)) assert.deepEqual(khoa==='lua_chon_json'?json(m[khoa]):m[khoa],khoa==='lua_chon_json'?json(ban[khoa]):ban[khoa]);
      }
      soNhom++; console.log('Đạt: '+bangCu+' trong JSON ('+cu.length+' bản ghi)');
    }
    await doiChieu('bước giá', `SELECT * FROM ${nguon}.buoc_gia ORDER BY gia_tu,id`, `SELECT gia_tri_cau_hinh FROM ${dich}.cau_hinh_he_thong WHERE khoa_cau_hinh='BUOC_GIA'`, rows=>json(rows[0].gia_tri_cau_hinh));
    const [cu] = await c.query(`SELECT * FROM ${nguon}.gia_han_phien_dau_gia ORDER BY id`);
    const [moi] = await c.query(`SELECT * FROM ${dich}.nhat_ky_hoat_dong WHERE hanh_dong='GIA_HAN_PHIEN' ORDER BY id`);
    assert.equal(moi.length,cu.length);
    for (const ban of cu) {
      const m = moi.find(x=>String(json(x.du_lieu_moi).gia_han_id_cu)===String(ban.id));
      assert(m); const j=json(m.du_lieu_moi);
      ...
      for (const cot of ['thoi_gian_ket_thuc_cu','thoi_gian_ket_thuc_moi']) assert.equal(j[cot].replace(/\.0+$/, ''),ban[cot].replace(/\.0+$/, ''));
      assert.equal(Number(j.so_giay_them),Number(ban.so_giay_them));
    }
    soNhom++; console.log('Đạt: lịch sử gia hạn ('+cu.length+' bản ghi)');
    console.log('Đối chiếu đủ '+soNhom+' nhóm dữ liệu gộp.');
  } finally { c.release(); }
}
main().catch(e=>{console.error('Đối chiếu thất bại:',e.code||e.name);process.exitCode=1;}).finally(()=>pool.end());
