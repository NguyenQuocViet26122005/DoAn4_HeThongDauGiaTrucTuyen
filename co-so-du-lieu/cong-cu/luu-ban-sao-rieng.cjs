...
async function main() {
  ...
  try {
    ...
    const banSao = { phien_ban: 1, database: nguon, ngay_luu: new Date().toISOString(), bang: [], views: [], triggers: [] };
    for (const ten of trangThai.thu_tu_bang_cu) {
      ...
      const [cot] = await c.query('SELECT COLUMN_NAME,DATA_TYPE FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=? AND TABLE_NAME=? ORDER BY ORDINAL_POSITION',[nguon,ten]);
      banSao.bang.push({ ten, ddl: ddl['Create Table'], cot, dong });
    }
    const [views] = await c.query('SELECT TABLE_NAME FROM information_schema.VIEWS WHERE TABLE_SCHEMA=?',[nguon]);
    for (const v of views) {
      const [[ddl]] = await c.query(`SHOW CREATE VIEW \`${nguon}\`.\`${v.TABLE_NAME}\``);
      banSao.views.push(ddl['Create View']);
    }
    const [triggers] = await c.query('SELECT TRIGGER_NAME FROM information_schema.TRIGGERS WHERE TRIGGER_SCHEMA=?',[nguon]);
    ...
    console.log('Đã lưu 27 bảng, '+views.length+' view, '+triggers.length+' trigger vào bản sao riêng; checksum khớp.');
  } finally {c.release();}
}
main().catch(e=>{console.error('Không lưu được bản sao:',e.code||e.name);process.exitCode=1;}).finally(()=>pool.end());
