...
async function main() {
  ...
  assert.equal(
    crypto.createHash('sha256').update(noiDung).digest('hex'),
    fs.readFileSync(tep + '.sha256', 'utf8').trim(),
  );
  ...
  try {
    ...
    try {
      for (const b of banSao.bang) {
        ...
        for (const d of b.dong) {
          const giaTri = b.cot.map((x) =>
            d[x.COLUMN_NAME] == null
              ? null
              : x.DATA_TYPE === 'json'
                ? typeof d[x.COLUMN_NAME] === 'string'
                  ? d[x.COLUMN_NAME]
                  : JSON.stringify(d[x.COLUMN_NAME])
                : d[x.COLUMN_NAME],
          );

          await c.execute(
            `INSERT INTO \`${b.ten}\` (${cot.map((x) => '`' + x + '`').join(',')}) VALUES (${cot.map(() => '?').join(',')})`,
            giaTri,
          );
        }
      }
    ...
    for (const ddl of banSao.views) {
      await c.query(doiSchema(ddl));
    }
    for (const ddl of banSao.triggers) {
      await c.query(doiSchema(ddl));
    }
    for (const b of banSao.bang) {
      const cot = b.cot.map((x) => '`' + x.COLUMN_NAME + '`').join(',');
      const [dong] = await c.query(
        `SELECT SHA2(CAST(JSON_ARRAY(${cot}) AS CHAR),256) AS ma FROM \`${b.ten}\` ORDER BY id`,
      );
    ...
    fs.writeFileSync(
      path.join(thuMuc, 'kiem-tra-phuc-hoi.json'),
...
main()
  .catch((e) => {
    console.error('Phục hồi chưa hoàn tất:', e.code || e.name);
    process.exitCode = 1;
  })
  .finally(() => pool.end());
