...
const cacBang = new Map([...sqlCu.matchAll(/CREATE TABLE (\w+) \(([\s\S]*?)\) ENGINE=InnoDB;/g)]
  .map(k => [k[1], k[0].replace(/\r\n/g, '\n')]));
if (cacBang.size !== 27) throw new Error('Schema nguồn không còn là 27 bảng đã đối chiếu.');
function thay(bang, cu, moi) {
  const sql = cacBang.get(bang);
  if (!sql?.includes(cu)) throw new Error(`Không tìm thấy vị trí sửa trong ${bang}`);
...
them('phien_dau_gia', `    -- Phí cố định; 0 nghĩa là miễn phí. Chụp nguyên số tiền sang đơn.
    phi_van_chuyen DECIMAL(15,2) NOT NULL DEFAULT 0,`);
rangBuoc('phien_dau_gia', `    CONSTRAINT ck_phien_phi CHECK (phi_van_chuyen >= 0),
...
them('don_hang', `    -- Giữ tiền mô phỏng, mỗi đơn chỉ quyết toán cuối một lần.
...
rangBuoc('don_hang', `    CONSTRAINT ck_don_tong CHECK (tong_tien = gia_san_pham + phi_van_chuyen),
...
them('thanh_toan', `    -- Một thao tác gửi lại dùng cùng khóa; lần thử mới dùng khóa mới.
...
for (const [bang, cot, tenCot, dieuKien] of [
  ...
  ['thanh_toan', 'don_hang_id', 'don_da_thu_tien', "trang_thai IN ('DA_THANH_TOAN','DA_HOAN_TIEN')"],
  ['tranh_chap', 'don_hang_id', 'don_dang_tranh_chap', "trang_thai IN ('DANG_MO','NGUOI_BAN_DA_PHAN_HOI','QUAN_TRI_DANG_XU_LY')"],
  ['de_nghi_mua_tiep_theo', 'phien_dau_gia_id', 'phien_dang_de_nghi', "trang_thai = 'CHO_XU_LY'"],
]) {
  them(bang, `    ${tenCot} BIGINT UNSIGNED GENERATED ALWAYS AS
        (CASE WHEN ${dieuKien} THEN ${cot} ELSE NULL END) STORED,`);
  rangBuoc(bang, `    UNIQUE KEY uq_${tenCot} (${tenCot})`);
  const b = cacBang.get(bang);
  const re = new RegExp('(FOREIGN KEY \\(' + cot + '\\) REFERENCES [\\s\\S]*?ON DELETE RESTRICT ON UPDATE )CASCADE');
  if (!re.test(b)) throw new Error('Không tìm thấy FK dùng cho cột sinh: ' + bang);
  cacBang.set(bang, b.replace(re, '$1RESTRICT'));
}
them('thong_bao', `    -- NULL dành cho dữ liệu cũ; nghiệp vụ mới đặt khóa theo sự kiện và đối tượng.
...
them('vi_pham', `    -- Điểm cũ chỉ để đối chiếu, không dùng để tự phạt.
...
rangBuoc('vi_pham', `    CONSTRAINT fk_vi_pham_nguoi_xu_ly FOREIGN KEY (nguoi_xu_ly_id) REFERENCES nguoi_dung(id)
        ON DELETE RESTRICT ON UPDATE CASCADE`);
them('de_nghi_mua_tiep_theo', `    -- Giá có nguồn là một lượt công khai; không liên kết với bảng mức tối đa.
...
rangBuoc('de_nghi_mua_tiep_theo', `    CONSTRAINT fk_de_nghi_luot_cong_khai FOREIGN KEY (luot_tra_gia_nguon_id) REFERENCES luot_tra_gia(id)
        ON DELETE RESTRICT ON UPDATE CASCADE,
...
rangBuoc('luot_tra_gia', '    INDEX idx_luot_ung_vien (phien_dau_gia_id, nguoi_tra_gia_id, ngay_tao, id)');
...
for (const ten of ['thuoc_tinh_danh_muc','gia_tri_thuoc_tinh_san_pham','hinh_anh_san_pham',
  'bang_chung_tranh_chap','muc_gia_toi_da','danh_sach_theo_doi','buoc_gia','gia_han_phien_dau_gia']) cacBang.delete(ten);
them('danh_muc', `    -- Mảng định nghĩa: khóa, tên, kiểu nhập, đơn vị, lựa chọn, bắt buộc.
    cau_hinh_thuoc_tinh JSON NULL,`);
rangBuoc('danh_muc', `    CONSTRAINT ck_danh_muc_thuoc_tinh CHECK
        (cau_hinh_thuoc_tinh IS NULL OR JSON_TYPE(cau_hinh_thuoc_tinh)='ARRAY')`);
them('san_pham', `    -- Object theo khóa; lưu cả giá trị và tên/kiểu/đơn vị đã chụp lúc duyệt.
    thuoc_tinh_json JSON NULL,`);
rangBuoc('san_pham', `    CONSTRAINT ck_san_pham_thuoc_tinh CHECK
...
rangBuoc('cau_hinh_he_thong', `    CONSTRAINT ck_cau_hinh_json CHECK (kieu_du_lieu<>'JSON' OR JSON_VALID(gia_tri_cau_hinh))`);
them('nhat_ky_hoat_dong', `    -- FK tường minh để lịch sử gia hạn không chỉ là dòng log văn bản.
    phien_dau_gia_id BIGINT UNSIGNED NULL,
    luot_tra_gia_id BIGINT UNSIGNED NULL,`);
rangBuoc('nhat_ky_hoat_dong', `    CONSTRAINT fk_nhat_ky_phien FOREIGN KEY (phien_dau_gia_id) REFERENCES phien_dau_gia(id)
        ON DELETE RESTRICT ON UPDATE RESTRICT,
...
cacBang.set('tham_gia_phien', `CREATE TABLE tham_gia_phien (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
cacBang.set('tep_dinh_kem', `CREATE TABLE tep_dinh_kem (
    id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
...
const nhom = [
  {id:'01-tai-khoan', ten:'Tài khoản', bang:['nguoi_dung','xac_minh_nguoi_ban','dia_chi_nguoi_dung']},
  {id:'02-san-pham', ten:'Danh mục và sản phẩm', bang:['danh_muc','san_pham']},
  {id:'03-dau-gia', ten:'Đấu giá', bang:['phien_dau_gia','tham_gia_phien','luot_tra_gia']},
  {id:'04-giao-dich', ten:'Đơn hàng và thanh toán', bang:['don_hang','thanh_toan','de_nghi_mua_tiep_theo']},
  {id:'05-hau-mai', ten:'Tranh chấp và tệp', bang:['tranh_chap','tep_dinh_kem','danh_gia']},
  {id:'06-quan-tri', ten:'Quản trị và thông báo', bang:['yeu_cau_xu_ly','vi_pham','thong_bao','cau_hinh_he_thong','nhat_ky_hoat_dong']},
];
// Đưa yêu cầu sau phiên; các nhóm khác giữ thứ tự khóa ngoại gốc.
const thuTu = nhom.flatMap(n => n.bang);
if (thuTu.length !== 19 || new Set(thuTu).size !== 19) throw new Error('Sai số bảng.');
...
`;
let sql = dau;
for(const n of nhom) sql += `-- ===== ${n.ten.toUpperCase()} (${n.bang.length} bảng) =====\n\n` + n.bang.map(b=>cacBang.get(b)).join('\n\n') + '\n\n';
let triggers = [...sqlCu.matchAll(/CREATE TRIGGER (\w+)[\s\S]*?END\$\$/g)].map(m=>m[0].replace(/\r\n/g,'\n'));
triggers = triggers.map(t => t.includes('ON muc_gia_toi_da')
  ? t.replace('ON muc_gia_toi_da','ON tham_gia_phien').replace('IF EXISTS (','IF NEW.gia_toi_da IS NOT NULL AND EXISTS (').replaceAll('NEW.nguoi_tra_gia_id','NEW.nguoi_dung_id') : t);
triggers = triggers.flatMap(t => [t, t.replace(/(CREATE TRIGGER \w+)/,'$1_sua').replace('BEFORE INSERT','BEFORE UPDATE')]);
...
BEGIN
    ...
    END IF;
END$$`;
triggers.push(baoVeDeNghi, baoVeDeNghi.replace('trg_de_nghi_gia_cong_khai','trg_de_nghi_gia_cong_khai_sua').replace('BEFORE INSERT','BEFORE UPDATE'));
sql += '-- Bảo vệ cơ bản ở CSDL; không thay thế khóa/thuật toán/phân quyền ở service.\nDELIMITER $$\n\n' + triggers.join('\n\n') + '\n\nDELIMITER ;\n\n';
const views = [...sqlCu.matchAll(/CREATE VIEW (\w+) AS[\s\S]*?;/g)].map(m=>m[0].replace(/\r\n/g,'\n'));
sql += views.join('\n\n') + '\n';
fs.writeFileSync(path.join(thuMuc, '01-tao-csdl-19-bang.sql'), sql);
const moHinh = nhom.map(n=>({...n, bang:n.bang.map(ten=>{
 const ddl=cacBang.get(ten);
 const cot=[...ddl.matchAll(/^    (\w+) (BIGINT|VARCHAR|ENUM|DATETIME|TIMESTAMP|TINYINT|INT|DECIMAL|JSON|TEXT)\b([^\n]*)/gm)]
    .map(m=>({ten:m[1],kieu:m[2],moTa:m[3].trim()}));
 const lienKet=[...ddl.matchAll(/FOREIGN KEY \((\w+)\) REFERENCES (\w+)\((\w+)\)/g)]
    .map(m=>({cot:m[1],bangCha:m[2],cotCha:m[3]}));
 return {ten,cot,lienKet};
})}));
fs.writeFileSync(path.join(thuMuc,'mo-hinh.json'),JSON.stringify(moHinh,null,2)+'\n');
console.log(JSON.stringify({bang:thuTu.length,khoaNgoai:moHinh.flatMap(n=>n.bang).reduce((s,b)=>s+b.lienKet.length,0),trigger:triggers.length,view:views.length}));
