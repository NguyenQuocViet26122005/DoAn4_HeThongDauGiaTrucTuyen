...
export function TrangThai({ giaTri }: { giaTri: unknown }) {
  ...
  const mau = [
    ...
    ? 'green'
    : ['DA_HUY', 'THAT_BAI', 'TU_CHOI', 'BI_KHOA', 'HET_HAN'].includes(ten)
      ? 'red'
      : 'gold';
...
export function TieuDe({
  ...
  return (
    <div className="tieu-de-trang">
      <div>
        {nhanNho && <span className="nhan-nho">{nhanNho}</span>}
        <h1>{ten}</h1>
        {moTa && <p>{moTa}</p>}
      </div>
      {children}
  ...
  if (truyVan.isError) {
    return (
      <Alert
        ...
        description={loiDeDoc(truyVan.error)}
        action={<Button onClick={() => truyVan.refetch()}>Thử lại</Button>}
      />
    );
  }
  if (rong) {
...
export function AnhSanPham({
  ...
  return (
    <div className={`anh-san-pham ${lon ? 'anh-lon' : ''}`}>
      {url && loi !== url ? (
        <img src={url} alt={ten} onError={() => datLoi(url)} loading="lazy" />
      ) : (
...
export function DemNguoc({ batDau, ketThuc }: { batDau?: string; ketThuc: string }) {
  ...
  const soGiay = Math.max(
    0,
    Math.floor((mocThoiGian(chuaBatDau ? batDau : ketThuc) - hienTai) / 1000),
  );

  if (!soGiay) {
    return <span className="chu-mo">Đã hết thời gian</span>;
  }

  const ngay = Math.floor(soGiay / 86400);
  const phan = [Math.floor(soGiay / 3600) % 24, Math.floor(soGiay / 60) % 60, soGiay % 60]
    .map((x) => String(x).padStart(2, '0'))
    .join(' : ');

  return (
    <span className={`dem-nguoc ${soGiay < 60 ? 'sap-het' : ''}`}>
      <BieuTuong ten="thoiGian" size={14} />
      {chuaBatDau ? 'Bắt đầu sau ' : ''}
      {ngay ? `${ngay}n ` : ''}
      {phan}
    </span>
  );
...
export function BieuMau({
  ...
  truong: TruongNhap[];
  banDau?: BanGhi;
  onLuu: (duLieu: BanGhi) => Promise<void>;
  nut?: string;
  children?: ReactNode;
}) {
    ...
    >
      {truong.map((t) => (
        <Form.Item
          ...
          name={t.ten}
          label={t.nhan}
          valuePropName={t.loai === 'switch' ? 'checked' : 'value'}
          rules={[
            { required: t.batBuoc, message: `Vui lòng nhập ${t.nhan.toLowerCase()}` },
            ...(t.loai === 'email'
        ...
        >
          {t.loai === 'select' ? (
            <Select
              options={t.luaChon}
              placeholder={`Chọn ${t.nhan.toLowerCase()}`}
              allowClear={!t.batBuoc}
            />
          ) : t.loai === 'switch' ? (
            <Switch />
          ) : t.loai === 'textarea' ? (
            ...
            <InputNumber min={t.min ?? 0} max={t.max ?? 9999999999999} style={{ width: '100%' }} />
          ) : t.loai === 'password' ? (
            <Input.Password autoComplete="current-password" />
          ) : (
            <Input type={t.loai || 'text'} maxLength={500} />
          )}
        </Form.Item>
      ))}
      {children}
      {loi && <Alert type="error" showIcon title={loi} className="loi-bieu-mau" />}
      <Button type="primary" htmlType="submit" loading={dangLuu} size="large">
        {nut}
        <BieuTuong ten="muiTen" size={16} />
      </Button>
    </Form>
  );
}
...
export function HopThoai({
  ...
  return (
    <>
      <Button danger={nguyHiem} onClick={() => datMo(true)}>
        {ten}
      </Button>
      <Modal title={ten} open={mo} onCancel={() => datMo(false)} footer={null} destroyOnHidden>
        <BieuMau
          truong={truong}
          banDau={banDau}
          nut="Xác nhận"
...
export function TaiTep({
  ...
  return (
    <label className="chon-tep">
      <BieuTuong ten="cong" />
      <span>{dangTai ? 'Đang tải lên…' : value ? 'Đã tải tệp · Chọn lại' : tieuDe}</span>
      <small>
        {nhom === 'evidence' ? 'Ảnh / PDF · tối đa 10 MB' : 'PNG, JPG, WebP · tối đa 5 MB'}
      </small>
      <input
        aria-label={tieuDe}
        type="file"
        ...
        onChange={async (e) => {
          ...
          if (tep.size > (nhom === 'evidence' ? 10 : 5) * 1024 * 1024) {
            message.error('Tệp vượt kích thước cho phép');
...
export function TepRiengTu({ url, ten }: { url: string; ten: string }) {
  ...
  return (
    <Button
      loading={dangTai}
      onClick={async () => {
        if (!url.startsWith('/api/uploads/files/')) {
          message.info('Tệp mẫu chưa có trên máy chủ. Hãy tải lên tệp mới.');
          ...
          modal.info({
            ...
            content:
              tep.data.type === 'application/pdf' ? (
                <iframe title={ten} src={blob} style={{ width: '100%', height: 500 }} />
              ) : (
                <img src={blob} alt={ten} style={{ maxWidth: '100%', maxHeight: 520 }} />
              ),
            afterClose: () => URL.revokeObjectURL(blob),
          });
        } catch (e) {
          message.error(loiDeDoc(e));
        } finally {
