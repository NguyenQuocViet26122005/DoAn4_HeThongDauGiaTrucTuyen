...
export const boNho = new QueryClient({ defaultOptions: { queries: { staleTime: 20000, retry: 1, refetchOnWindowFocus: true } } });
...
http.interceptors.request.use((cauHinh) => {
  ...
  if (token) cauHinh.headers.Authorization = `Bearer ${token}`;
  return cauHinh;
});
...
export async function gui<T = unknown>(url: string, noiDung: unknown = {}, phuongThuc: 'post' | 'put' | 'patch' | 'delete' = 'post'): Promise<T> {
  return (await http.request<{data: T}>({ url, method: phuongThuc, data: noiDung })).data.data;
}
...
export function loiDeDoc(loi: unknown): string {
  if (axios.isAxiosError(loi)) return loi.response?.data?.message || 'Chưa kết nối được máy chủ. Vui lòng thử lại.';
...
export function duongDanAnh(giaTri?: string | null): string | undefined {
  if (!giaTri || (!giaTri.startsWith('/api/uploads/files/') && !/^https?:\/\//.test(giaTri))) return undefined;
  if (giaTri.startsWith('/api/') && import.meta.env.VITE_API_URL) return import.meta.env.VITE_API_URL.replace(/\/api\/?$/, '') + giaTri;
  return giaTri;
}
